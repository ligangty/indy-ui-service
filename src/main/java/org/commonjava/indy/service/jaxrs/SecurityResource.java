...
import org.commonjava.indy.service.controller.SecurityController;
...
public class SecurityResource
{
    ...
    @APIResponse( responseCode = "400", description = "Keycloak is disabled" )
    ...
    @APIResponse( responseCode = "307", description = "Redirect to keycloak server to load Javascript adapter." )
    @Path( "/keycloak.js" )
    @Produces( "text/javascript" )
    @GET
    ...
    private Response formatResponse( final Throwable error )
    {
        final String id = generateErrorId();
        final String msg = formatEntity( id, error ).toString();
        Response.Status code = Response.Status.INTERNAL_SERVER_ERROR;

        if ( ( error instanceof IndyUIException ) && ( (IndyUIException) error ).getStatus() > 0 )
        {
            final int sc = ( (IndyUIException) error ).getStatus();
    ...
    private CharSequence formatEntity( final String id, final Throwable error )
    {
